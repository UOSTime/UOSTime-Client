...
export default function FindPWDialog({onClose, open}) {
  ...
  const onClick = async () => {
    ...
    if(!input.email) {
      setResult({send: false, emailError: '이메일을 입력해주세요!', usernameError: ''});
      return;
    } else if(!input.username) {
      setResult({send: false, emailError: '', usernameError: '아이디를 입력해주세요!'});
      return;
    }
    if(result.send) {
    ...
    setIsLoading(true);
    const response = await requestAPI(API_FIND_PW().setQuery({email: input.email, uid: input.username}));
    ...
    if(response.status === StatusCodes.OK) {
      ...
      switch (response.status) {
        case 400:
          message = '클라이언트에서 오류가 발생했어요...'; break;
        case 404:
          message = '해당 이메일로 등록된 계정이 없어요'; break;
        default:
          message = '서버에서 오류가 발생했어요...'; break;
      }
