...
export default function SignUpDialog({ onClose, open }) {
  ...
  const onSubmit = async () => {
    ...
    setResult({ send: response.status === StatusCodes.CREATED, error: '' });
    setIsLoading(false);
  };

  const errorMessage = result.error ? (
    <Typography className={fontClasses.warning}>{result.error}</Typography>
  ) : null;
  const resultMessage = result.send ? (
    <Typography className={fontClasses.blue}>
      회원가입이 완료되었어요!
    </Typography>
  ) : null;
      ...
      <Container className={classes.content}>
        <Container>
          <UosInput
            ...
            value={newUser.uid}
          />
          <Typography className={fontClasses.default}>
            4 ~ 20자의 영문자, 숫자, 특수기호(_)만 사용해주세요.
          </Typography>
        </Container>
        <Container>
          <UosInput
        ...
        {resultMessage}
        {errorMessage}
        <Button className={buttonClasses.linearRed} onClick={onSubmit}>
          회원가입
        </Button>
      </Container>
      {loading}
